...
export default function RecentActivity({
  ...
  return (
    <Card className="bg-white dark:bg-[#0f172a] border border-gray-300 dark:border-gray-700/40 shadow-lg">
      <CardHeader className="border-b border-gray-300 dark:border-gray-700/40">
        <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
          <span className={`w-7 h-7 rounded-md ring-1 flex items-center justify-center ${headerBg}`}>
            <Icon className={`w-4 h-4 ${accent}`} />
      ...
      <CardContent className="p-0">
        {items.map((item, index) => (
          <div
            key={item.id}
            className={`p-4 transition-colors hover:bg-gray-100 dark:hover:bg-[#1a253b] ${
