...
function ResumeList({ resumes }: { resumes: Resume[] }) {
  const getScoreStyle = (score: number) => {
    if (score >= 80) return 'bg-emerald-900/40 text-emerald-300 border-emerald-500'
    if (score >= 60) return 'bg-amber-900/40 text-amber-300 border-amber-500'
    return 'bg-rose-900/40 text-rose-300 border-rose-500'
  }
  ...
  if (resumes.length === 0) {
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-10 text-center shadow-md w-full">
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      {resumes.map((resume) => (
        <div
          key={resume.id}
          className="bg-gray-900 border border-gray-700 rounded-xl p-6 shadow-md hover:shadow-lg w-full"
        >
          <div className="flex flex-col md:flex-row justify-between gap-6 w-full">
            <div className="flex-1 w-full">
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-300">{resume.targetRole}</p>
              ...
              <div className="space-y-3">
                {resume.strengths?.length > 0 && (
                  <div className="rounded-lg border border-emerald-700 bg-emerald-900/20 p-4">
                    <h4 className="font-semibold text-emerald-300 mb-2 flex items-center gap-2 text-sm uppercase tracking-wide">
                ...
                {resume.weaknesses?.length > 0 && (
                  <div className="rounded-lg border border-rose-700 bg-rose-900/20 p-4">
                    <h4 className="font-semibold text-rose-300 mb-2 flex items-center gap-2 text-sm uppercase tracking-wide">
                      <AlertTriangle className="w-4 h-4" /> Improvements
                    </h4>
                    <ul className="text-sm text-rose-300 space-y-1 list-disc list-inside">
                ...
                {resume.suggestions?.length > 0 && (
                  <div className="rounded-lg border border-amber-700 bg-amber-900/20 p-4">
                    <h4 className="font-semibold text-amber-300 mb-2 flex items-center gap-2 text-sm uppercase tracking-wide">
            ...
            <div className="flex flex-row md:flex-col gap-3 md:w-40">
              <Button
                ...
                className="bg-gray-800 border border-gray-700 hover:border-gray-600 text-white font-medium rounded-md shadow-sm hover:shadow transition-colors"
              >
                <ExternalLink className="w-4 h-4 mr-2" /> View
              </Button>
              <Button
                onClick={() => copyShareLink(`${window.location.origin}/resume/${resume.id}`)}
                className="bg-blue-700 hover:bg-blue-800 text-white font-medium rounded-md shadow-sm hover:shadow"
              >
